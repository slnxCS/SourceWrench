...
public class ExportModelCommand : ICommand
{
    ...
    public ExportModelCommand(ModelExtractor modelExtractor)
    {
        _modelExtractor = modelExtractor;
    }

    public string Name => "exportmodel";
    public string Description => "Extract model files (.mdl, .vvd, .vvdx, .phy, .vtx, .vpd, .ani, .dx90.vtx, .dx80.vtx, .sw.vtx) from a VPK archive";
    ...
    public async Task<int> ExecuteAsync(string[] args, CancellationToken cancellationToken = default)
    {
        ...
        return await _modelExtractor.ExtractAsync(sourcePath, outputPath, cancellationToken);
    }
}